...
<Info>
  ...
  using the default auto-py runtime, or adding an entrypoint to the runtime
...
System-level packages provide the foundation for many ML apps, handling everything from image-processing libraries to audio codecs. These can be added to the `cerebrium.toml` file under the `[deps.apt]` section as follows:
...
For teams with standardized system dependencies, text files can be used instead by adding the following to the `[deps.paths]` section:
...
While Cerebrium's default auto-py runtime works well for most apps, teams often need more control over their server implementation. Custom runtimes enable features like custom authentication, dynamic batching, public endpoints, or WebSocket connections.
...
<Warning>
  ...
  handled within the Dockerfile. The `[deps.*]` sections will be ignored.
</Warning>
