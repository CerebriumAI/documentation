...
We have made all endpoints OpenAI compatible so in order to call the endpoint you can do the following in another file:
...
Above we set our base url to the one returned by our deploy command - it ends in /run since thats the function we are calling. Lastly, we use our JWT token which is returned in the 
CURL command when you deploy or can be found in your Cerebrium dashboard under the section API Keys.

Voila! You now have a OpenAI compatible endpoint that you can customize to your liking!
