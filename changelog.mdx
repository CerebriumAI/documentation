...
- We've switched out infrastructure provider for something more reliable and faster. This means faster deployment, less downtime and roughly half the inference time!
...
-  Added support for post-processing functions in s!
...
- Fix a bug where the internal  would be malformed if the model was not
  ONNX
- Force single model ONNX  until post-processing is supported
...
- Support for ONNX models with Python 3.7-3.9 (can be used as a single model or
  as the initial model of a ). We are working on supporting Python 3.10
  and 3.11!
