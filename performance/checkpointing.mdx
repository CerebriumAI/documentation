...
description: "Radically reduce container cold starts by skipping initialization work"
---
...
The larger the size of the memory checkpoint, the slower the restore. You can reduce the size of the snapshot and improve startup times by dropping the KV cache before checkpoint and recreating it after restore. vLLM provides built-in functionality for this through [vLLM Sleep Mode](https://docs.vllm.ai/en/latest/features/sleep_mode/).
