...
I then go straight to the [Hugging face documentation](https://huggingface.co/stabilityai/stable-diffusion-2-1) and see that to run Stable Diffusion 2.1
...
To deploy this function to an RTX_A5000 instance, we run the following code:

```bash
cerebrium deploy stable-diffusion --hardware RTX_A5000 --api-key private-XXXXXXXXXXXXX
```

We can then make the following request:

```bash
curl --location --request POST 'https://run.cerebrium.ai/v2/p-xxxxx/stable-diffusion/predict' \
