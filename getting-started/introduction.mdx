---
title: Introduction to Cerebrium for real-time AI workloads
description: Cerebrium is a serverless GPU platform for real-time and high-performance AI apps with low cold starts, burst scaling, and global low-latency inference.
---
...
Run PowerShell as Administrator, then download and extract the CLI:
...
Expand-Archive -Path "cerebrium.zip" -DestinationPath "$env:LOCALAPPDATA\Cerebrium"
```

The extracted `cerebrium.exe` is not on your `PATH` by default, so `cerebrium login` will fail with `CommandNotFoundException` until you add it. Add the install folder to `PATH` for the current user:

```powershell
[Environment]::SetEnvironmentVariable("Path", $env:Path + ";$env:LOCALAPPDATA\Cerebrium", "User")
```

Close and reopen PowerShell, then confirm the CLI is available:
