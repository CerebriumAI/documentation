...
| entrypoint           | string[] | -        | Command to start the application. Required for Python ASGI/WSGI apps or if Dockerfile has no CMD. **Always takes precedence over Dockerfile CMD when specified.** |
...
| dockerfile_path      | string   | -        | Relative path to a custom Dockerfile. When specified, your Dockerfile is used for building the container image   |
...
<Info>
  **Entrypoint Precedence:** When both `entrypoint` in `cerebrium.toml` and `CMD` in your Dockerfile are defined, the TOML `entrypoint` always takes precedence. This allows you to override your Dockerfile's default command at deploy time without modifying the Dockerfile itself.
</Info>
