...
### Increasing the Initialization Timeout

If your app loads large models or does heavy setup at startup, the build can fail with an initialization timeout. Raise `deployment_initialization_timeout` up to the 830-second maximum:
...
This timeout only applies to app initialization during build. The `response_grace_period` option in the [scaling configuration](#scaling-configuration) controls request timeouts during inference.
...
| compute   | string or string[] | "CPU"             | Compute type, or a list of acceptable compute types in preference order            |
| gpu_count | integer            | 0                 | Number of GPUs                                                                     |
| provider  | string             | platform-selected | Cloud provider. Omit to let the platform choose                                    |
| region    | string             | platform-selected | "global" to run in any region with available capacity, or a specific region to pin |
...
<Info>
  `compute` accepts a single type or a preference-ordered list, e.g. `compute =
  ["HOPPER_H100", "AMPERE_A100_80GB"]`. See [GPU preference
  lists](/hardware/using-gpus#gpu-preference-lists). Placement is covered in
  [Multi-Region Deployment](/deployments/multi-region-deployment).
</Info>
