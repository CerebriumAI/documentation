...
description: "Reduce Cerebrium cold start times with Tensorizer, persistent storage, and container packaging strategies for loading model weights into GPU memory."
keywords: "cold start, snapshot, memory snapshot, warm pool, preloaded model, tensorizer"
---

<Note>
  Looking for memory snapshots, container snapshots, or a warm pool of preloaded
  containers? See [Memory Checkpointing](/other-topics/checkpointing), which
  snapshots a ready container's GPU and CPU memory so subsequent containers
  restore instantly.
</Note>
