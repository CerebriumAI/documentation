...
```python
def predict(prompt, cutoff_len, temperature, top_p, top_k, max_new_tokens, run_id):
...
```curl
curl --location --request POST 'https://api.cortex.cerebrium.ai/v4/p-xxxxxx/streaming-falcon/stream' \
