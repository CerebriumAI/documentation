...
Apps can process multiple requests simultaneously using Cerebrium's batching and concurrency features. The system offers native support for frameworks with built-in batching capabilities and enables custom implementations through the [custom runtime](container-images/defining-container-images#custom-runtimes) feature. For detailed information about handling multiple requests efficiently, see our [Batching & Concurrency Guide](/scaling/batching-concurrency).
...
<Note>
  ...
  guide](/scaling/graceful-termination) for detailed implementation
...
| Tier            | Description                                                                                                                        |
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `interruptible` | **(Default)** Scheduled on spot instances. Lower cost, but may be interrupted by the cloud provider during periods of high demand. |
| `protected`     | Scheduled on on-demand instances. Higher availability with no interruptions, at increased cost.                                    |
