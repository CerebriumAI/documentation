...
- Since we need a GPU, we need to use one of the base images that come from Nvidia that has the CUDA libraries installed. We use the Cuda 12 image. You can see other images [here](/cerebrium/container-images/defining-container-images).
- Depending on the type of CPU/GPU you need, you can update the hardware settings to run your app. You can see the available options in the [GPU guide](/cerebrium/hardware/using-gpus) and [CPU and memory guide](/cerebrium/hardware/cpu-and-memory).
...
- [Secrets](/cerebrium/other-topics/using-secrets)
- [Model scaling](/cerebrium/scaling/scaling-apps)
- [Faster cold starts](/cerebrium/other-topics/faster-cold-starts)
