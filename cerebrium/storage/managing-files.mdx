...
<Note>
  The `/persistent-storage` directory is not directly accessible during development or outside of the Cerebrium environment - you'll need to use the Cerebrium CLI commands to manage these files.
</Note>
...
<Note>
  When using these commands, the Cerebrium CLI does not display the `/persistent-storage/` portion of the path in its output - it shows relative paths from the persistent storage root.
</Note>
