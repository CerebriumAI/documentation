...
<Info>
  `eu-north-1` (Stockholm) and `eu-north1` (Finland) are different regions. Some
  regions run on cloud providers other than AWS: when pinning `us-east1` or
  `us-southcentral1-a`, set `provider = "crusoe"`; when pinning `us-central1` or
  `eu-north1`, set `provider = "nebius"`.
</Info>
...
| us-east1                | BLACKWELL_B200, HOPPER_H200, HOPPER_H100, AMPERE_A100_80GB, AMPERE_A100_40GB                                                                     |
...
| eu-north-1 (on request) | HOPPER_H100, ADA_L40, ADA_L4, AMPERE_A10, TURING_T4, INF2, TRN1                                                                                  |
