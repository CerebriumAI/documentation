...
Cerebrium provides flexible scaling controls to balance performance, cost, and responsiveness. Understanding these parameters helps optimize the app for specific use cases.

### Basic Configuration

The fundamental scaling parameters are configured in the `cerebrium.toml` file:

```toml
[cerebrium.scaling]
min_replicas = 0       # Minimum running instances
max_replicas = 3       # Maximum concurrent instances
cooldown = 60          # Keep warm period (seconds)
```

### Keeping Models Warm

Cold starts can impact response times, especially for large ML models. Cerebrium offers two strategies to keep models ready for incoming requests:

### Strategy 1: Minimum Replicas

Setting `min_replicas` to 1 or higher ensures instances are always running:

```toml
[cerebrium.scaling]
min_replicas = 1    # Always keep one instance ready
```

Best for:

- Meeting customer SLAs
- Handling consistent base load
- Eliminating cold starts

<Warning>
    Note: This option incurs 24/7 charges as instances remain active continuously.
</Warning>

### Strategy 2: Cooldown Period

The `cooldown` parameter keeps instances warm for the specified period, after processing requests:

```toml
[cerebrium.scaling]
cooldown = 300    # Keep instances warm for 5 minutes after last request
```
...
Each new request resets the cooldown timer. Charges apply during the cooldown period as instances remain active.

### Scaling Limits

The `max_replicas` parameter controls concurrent instance limits:

```toml
[cerebrium.scaling]
max_replicas = 3    # Maximum of 3 instances running simultaneously
...
GitHub repositories require special handling - they should be installed via shell commands rather than pip dependencies to ensure proper version control and build reproducibility.
...
The base image is specified through the `docker_base_image_url` parameter:

```toml
# Default minimal image
docker_base_image_url = "debian:bookworm-slim"

# CUDA-enabled image for GPU workloads
docker_base_image_url = "nvidia/cuda:12.0.1-runtime-ubuntu22.04"
docker_base_image_url = "nvidia/cuda:12.0.1-devel-ubuntu22.04"
```

<Info>
    Cerebrium supports all public docker images that are compatible with Ubuntu, Cuda, and Python
</Info>

### Impact on Builds

Changing the base image has significant implications for the build process. It triggers a complete rebuild since it affects the foundation of the container environment. This means:

- All dependencies reinstall from scratch
- Build caches invalidate
- Initial deployment takes longer
- Subsequent builds remain cached until the base image changes again

### Build-time vs Runtime Considerations

When selecting a base image, consider both build and runtime impacts:

Build-time:

- Larger base images increase initial build time
- More included packages mean fewer explicit installations needed
- Some images come with pre-configured optimizations

Runtime:

- Image size affects cold start time
- Included libraries can improve performance
- GPU support enables hardware acceleration
...
<Info>
    Check out [this example](https://github.com/CerebriumAI/examples/tree/master/11-python-apps/1-asgi-fastapi-server)  for a detailed implementation of a FastAPI server that uses a custom runtime.
</Info>

### Running ASGI/WSGI Apps

The system supports both ASGI (like FastAPI) and WSGI apps. For ASGI apps, remember to install the appropriate server package (like uvicorn). The entrypoint command should match how you would normally run the app locally.
...
When requests arrive, Cerebrium automatically creates containers from this image. Each container runs independently, executing the runtime shell commands during startup. The system scales the number of containers up or down based on demand and specified scaling parameters, but all containers start from the most recent image.