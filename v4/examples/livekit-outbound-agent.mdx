...
description: "Create a Outbound AI agent that can transfer calls to real agents"
---

Voice agents are transforming how businesses operate introducing efficiencies and personalization for each customer interaction. While most use cases are focusing on agents receiving calls from customers, this article focusses on outbound voice AI agents and the use cases they unlock.
...
In this tutorial we will show you how you can set up an outbound calling agent that does a warm transfer - ie: can handover the call to a real person using Livekit. A fitting example scenario would be for a support center where teams would like an agent to do data collection before connecting to a real person .
...
Then create a **.env** file with your LiveKit credentials. In order to get these credentials, you can go to the [LiveKit dashboard](https://cloud.livekit.io/), click on the Settings tab in order to get your project url.
...
For our solution, we will use the following services in order to create our outbound AI agent:
...
In our use case, we would like to connect the customer to a real person when a specific stage has been reached or a user has asked to be connected. We can achieve this through function calling. In order to add function calling we can add the following code to our entrypoint.
...
Lastly, in order to create an outbound call to the user we would like to add to our call, we need to add functionality in order to do this. Add the following code to your [main.py](http://main.py) - above entrypoint():
...
In order to test this running locally, you can create a test.py file with the following functionality:
...
We need to specify our Cerebrium environment in order to deploy our application. In our `cerebrium.toml`, edit the values with the following contents:
...
In order to reduce latency, Cerebrium has partnerships with both Deepgram and Rime in order to run their STT and TTS models locally right next to your LiveKit worker which reduces latency by a ~400ms.
