...
-  Added support for post-processing functions in pipelines!
...
- Fix a bug where the internal pipeline would be malformed if the model was not
  ONNX
- Force single model ONNX pipeline until post-processing is supported
...
- Support for ONNX models with Python 3.7-3.9 (can be used as a single model or
  as the initial model of a pipeline). We are working on supporting Python 3.10
  and 3.11!
...
- Released Model/Pipeline Versioning
