...
You can also define a Conduit object directly by using the `Conduit` class. Then call the `run` method on the Conduit object to test the model locally, or the `deploy` method to deploy the model to Cerebrium.
```python
...
You can read about adding a logger [here](/Cerebrium/logging).
...
<ParamField body="data" type="array" required>
   The data you would like to send to your model. You may feed an ndarray or Tensor directly but it must be within an array.
</ParamField>
