...
Add the Hugging Face token to Secrets on Cerebrium as `HF_TOKEN`.
...
Run the following command to create the Cerebrium starter project: `cerebrium init pipecat-agent`. This creates two files:
...
The LLM is Llama 3 8B, served via vLLM. Request access to [Llama 8B](https://huggingface.co/meta-llama/Meta-Llama-3-8B-Instruct) on Hugging Face (approval typically takes less than 30 minutes).

Add the Hugging Face token as a secret named "HF_TOKEN" in the Cerebrium dashboard under "Secrets" for secure runtime access.
...
PipeCat requires models to follow the OpenAI-compatible format and doesn't support local instantiation, so vLLM runs as a local background process. The process is monitored for successful launch and retries after 5 seconds if needed (a known issue with rapid vLLM starts). The server runs on port 5000 (port 8000 is reserved for Cerebrium), with a persistent download directory to speed up subsequent cold starts.

Implement the Pipecat framework by creating a `main` function:
...
The `main` function initializes the Daily transport layer to handle audio/video data from the Daily room, passing the room URL and authentication token for programmatic joining. VAD stop seconds is set to 200 milliseconds, defining the pause duration before the bot responds.

The locally running Deepgram models (from the Docker base image on port 8082) handle audio-to-text and text-to-audio conversion via PipeCat. The local LLM connects from the vLLM server. Set the Deepgram API key to `None` to use the local instance.

Everything combines into a PipelineTask, which PipeCat executes. Tasks are fully customizable and support Image and Vision use cases ([docs](https://docs.pipecat.ai/docs/category/services)). Pipeline tasks include built-in interruption handling and easy model swapping.
...
Events attach to Transport (the meeting room communication method). The Pipeline task runs through the pipeline runner until signaled to exit (when a call ends). See [PipeCat's architecture docs](https://docs.pipecat.ai/docs/understanding-bots/dailyai-architecture) for more details.

The code runs in a separate execution environment to prevent multiple PipeCat instances. This background process serves as the entry point for the REST API endpoint to start the PipeCat bot. When the call ends and the bot returns, a response is sent to the API endpoint:
