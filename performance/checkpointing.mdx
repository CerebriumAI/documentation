...
If checkpoint creation succeeds, subsequent containers restore from that snapshot. A restored container logs a line beginning `CEREBRIUM_RESTORED` as its first log line, naming the snapshot size and how long the restore took:

```text
CEREBRIUM_RESTORED: container my-app restored from checkpoint (1228.8 MiB in 2.1s)
```

`GET /checkpoint/status` and `GET /checkpoint/wait` report the same restore in exact units:
...
`duration_ms` runs from the snapshot being ready to this container's first call to either endpoint, so it covers the restore plus however long the process took to reach that call. It excludes the time spent downloading the snapshot. Expect it to read slightly higher than the figure in the `CEREBRIUM_RESTORED` log line, which times the restore alone.
...
**Graphics APIs:** Vulkan, OpenGL, and EGL are not supported at all. The runtime does not enable the NVIDIA `graphics` driver capability, so an app that requires it either fails to start or fails at driver initialization.

**Video encode and decode:** The GPU video engines run, but an app holding a video engine session cannot be checkpointed. See [Video encode and decode](#video-encode-and-decode).
...
The GPU video engines cannot be checkpointed. This covers NVENC (encode), NVDEC (decode), NVJPG (JPEG), and the optical flow engine. The GPU checkpointer only captures CUDA compute state, so a live video engine session has no way to be saved and the snapshot is refused.

This applies even to a single-process app. The video session alone is enough — no `fork` and no worker processes are involved, so switching to `spawn` does not help.

Release the video session before triggering the checkpoint, and reopen it afterwards. If the app must hold a video engine open for its whole lifetime, it cannot be checkpointed today.

## Internal checkpoint error

The checkpoint failed inside the checkpointing runtime, not in your app. No change to your code prevents this one.

Email [support@cerebrium.ai](mailto:support@cerebrium.ai) with your app id and the time of the failed checkpoint. The underlying error is recorded on the node and can be matched against that timestamp.

## Checkpoint interrupted

The checkpoint stopped before it finished, usually because the container was replaced or the node drained while the snapshot was being written.

Trigger the checkpoint again once the app is warm. If every attempt ends the same way, email [support@cerebrium.ai](mailto:support@cerebrium.ai) with your app id.
