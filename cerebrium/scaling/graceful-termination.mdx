...
Cerebrium is a shared-compute, multi-tenant environment. To scale out, optimize compute packing, and perform updates, Cerebrium dynamically adjusts its capacity—shutting down nodes and bringing up new ones throughout the day. As this happens, user workloads are shifted to new nodes. In addition, your application has its own metric-based autoscaling criteria that dictate when instances should scale or remain active, as well as handle instance shifting during new app deployments. You may need to implement graceful shutdown to prevent requests from ending prematurely when app instances terminate, which commonly helps avoid 502 errors.
...
stateDiagram-v2
  ...
  Running --> SIGTERM: scale down / redeploy
  state "SIGTERM sent" as SIGTERM

  SIGTERM --> ifGrace
  state ifGrace <<choice>>

  ifGrace --> Waiting: response_grace_period configured
  ifGrace --> ForceKilled: no response_grace_period

  state "Waiting (grace period)" as Waiting
  Waiting --> GracefulShutdown: all requests complete
  Waiting --> ForceKilled: grace period timeout

  GracefulShutdown --> [*]: clean shutdown
  ForceKilled --> [*]: 502 errors

  note right of SIGTERM
    Cortex: Automatic
...
<Warning>
Missing this configuration is the most common cause of 502 errors during scaling.  
Set this to 1.5× your longest expected request duration**.
</Warning>
