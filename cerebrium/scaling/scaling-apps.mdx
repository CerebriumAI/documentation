...
replica_concurrency = 1    # The maximum number of requests an instance of an app can accept
```
...
The number of requests an app instance can handle is dictated by the `replica_concurrency` parameter. This is a hard limit, and the application will
not accept more than this limit. By default, once this concurrency limit is reached on an instance and there are still requests to be processed in-flight,
the system will scale out by the number of new instances required to fulfil the in-flight requests.

<Info>
  Typically most GPU applications will require that `replica_concurrency` is set
  to **1**. If changing this value, and the workload requires GPU, ensure that
  access to GPU resources is controlled within the application.
</Info>
...
`cpu_utilization` uses a maxmium CPU percentage utilization averaged over all instances of an application to scale out, relative to the
...
`memory_utilization` uses a maxmium memory percentage utilization averaged over all instances of an application to scale out, relative to the
`cerebrium.hardware.memory` value. For example, if an application has `memory=10` and `scaling_target=80`, Cerebrium will attempt
