...
This ensures that Cerebrium can handle even the most demanding workloads. If one of either of these two conditions are met, additional workers are spun up in < 3 seconds
...
If an instance heads into a bad state due to memory or processing issues, Cerebrium automatically restarts a new instance to handle the incoming load.
