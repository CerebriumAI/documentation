...
The example uses [PipeCat](https://www.pipecat.ai/) to handle component integration, user interruptions, and audio data processing.

Find the final version of the code [here](https://github.com/CerebriumAI/examples/tree/master/6-voice/4-twilio-voice-agent)
...
Replace the stream URL with the deployment's base endpoint, using the correct project ID.
...
Read more about running custom web servers [here](/container-images/custom-web-servers).
...
Save the changes and proceed to set up the AI agent.
...
3. Uses [Secrets](https://docs.cerebrium.ai/environments/using-secrets) for authentication
...
For lower latency (~500ms end-to-end), run parts or all of the pipeline locally. See the [voice agents guide](/v4/examples/realtime-voice-agents) and [RAG voice agent blog post](https://www.cerebrium.ai/blog/creating-a-realtime-rag-voice-agent).

### Deploy to Cerebrium

Deploy to Cerebrium by running `cerebrium deploy` in the terminal.
...
Update `cerebrium.toml` with the following:
