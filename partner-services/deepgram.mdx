---
title: Self-hosted Deepgram speech-to-text on Cerebrium
...
Obtain links to the Deepgram model files referenced below (file extension `.dg`) from your Deepgram Account Representative.

Consult your Deepgram representative on how to achieve parity with the Deepgram API.
...
<Info>
  ...
  integration matures. Reach out to [support](mailto:support@cerebrium.ai) if
  you hit any issues.
</Info>
...
<Note>
  ...
  storage and is shared across all Deepgram apps in the project. The configuration
  files (`api.toml` and `engine.toml`), however, belong under the app name
  directory (see steps 4 and 5 below).
</Note>
...
5. Create a file named `api.toml` with the following content and upload to your persistent storage under the app name directory (e.g., `{appName}/api.toml`). These are
   ...
   ```bash
   cerebrium cp api.toml deepgram/api.toml
   ```
...
6. Update `cerebrium.toml` with the following configuration for hardware, scaling, region, and other settings:
...
- Navigate to the **Secrets** tab in the Cerebrium dashboard
