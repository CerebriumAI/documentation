...
Once configured, API calls use the custom domain while keeping the same path structure: `api.example.com/v4/p-1234/my-app/run`.
...
4. Enter the domain name and select the target region (the region closest to users provides the lowest latency)
...
Cerebrium attempts to validate domains once every 30 minutes for up to 2 days. If validation has not succeeded, click "Validation Status" to see the last attempt and any errors.
...
- **Wrong proxy URL**: Use the exact URL shown in the dashboard
