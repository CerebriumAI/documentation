...
**Vulkan:** Graphics APIs that need the NVIDIA `graphics` driver capability cannot be checkpointed. See [Vulkan](#vulkan).

**Video encode and decode:** NVENC, NVDEC, and the NVIDIA `video` driver capability cannot be checkpointed. See [Video encode and decode](#video-encode-and-decode).
...
Vulkan, OpenGL, and EGL cannot be checkpointed. Those APIs need the NVIDIA `graphics` driver capability, and the GPU checkpointer only covers CUDA compute state.

Do not initialize a graphics API in an app that you intend to checkpoint. If a dependency requires Vulkan, the app cannot be checkpointed today.

## Video encode and decode

NVIDIA video encode and decode (NVENC, NVDEC, and the `video` driver capability) cannot be checkpointed.

Use a CUDA-only pipeline for apps that need checkpointing. If a dependency requires NVENC or NVDEC, the app cannot be checkpointed today.
