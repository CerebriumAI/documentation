...
In certain use-cases where app startup time or total request time is long and traffic is predicatable, a consistent excess capacity
which is dynamically added to the app's current required capacity may be needed in order to meet adequate request throughput.
