...
Logging with Arize is particularly easy with Cerebrium since you just have to define about 8 lines of code, and it follows an almost identical pattern to if you are familiar with how to log with Arize.
...
- `features`: A list of the column names containing your different features. Do not include outcome variable names — only features.
...
- `schema`: This is the schema that Arize requires to determine what your predictions, features and embeddings are.
...
With every request made to your Cerebrium endpoint, a unique `run_id` is returned. We use this `run_id` to associate every log of the output with your model. You can use this `run_id` to log the actual values to Arize and compare model performance to that of your baseline model.
...
If you are passing embedding features to your model, you must make sure to not include it in the feature array and define it separately.
