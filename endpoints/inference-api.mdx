...
Create and manage keys in the **API Keys** section of the dashboard, or with the [Create API Key](/api-reference/api-keys/create-api-key) endpoint. The secret value is returned once, at creation time.
...
- **Deletion**: Deleting a key is permanent. Requests using the deleted key start failing within a few seconds, and the key cannot be recovered. Only project owners can delete keys. Keys that belong to a service account are removed by deleting the service account.
