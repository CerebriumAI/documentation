...
<Note>
    When using the cortex runtime (default) the SIGTERM signal is captured and the app is given a chance to complete requests before being terminated.
    When using a custom runtime, it is the responsibility of the user to handle the SIGTERM signal and ensure that the app is given a chance to complete requests before being terminated.
</Note>
