...
- When you make changes to your deployment, the server will automatically reload your deployment and apply your changes without rebuilding the entire deployment. This can save you a lot of time when you are iterating on your deployment.
  - If you would like to get your serve session started faster, offload your model weights to a cloud storage platform (huggingface is great for this). This means your local build resources are smaller, avoiding long initial uploads.
...
- To end a `serve` session, simply press <<ctrl+c>> in your terminal where you started your serve session. This triggers a shutdown routine that will end your session.
