...
It is best look at the code in the [Github repo](https://github.com/lucataco/cog-sdxl-lightning-4step) and follow along as we migrate it.
...
- Depending on the type of CPU/GPU you need, you can update the hardware settings to run your application. You can see the full list available [here](https://docs.cerebrium.ai/available-hardware)
- We copy across the pip packages we need to install
- Replicate uses pget to download model weights - we therefore need to download it in order to use it. We do this by installing curl and then adding the shell commands in our cerebrium.toml
...
Replicate makes use of classes for their syntax which we shy away from - we run whatever python code you give us and make each function an endpoint. Therefore, when you see a reference to **self.** just remove it throughout the code
...
The code above downloads the models weights if they don’t exist and then instantiates the models. In order to persist files/data on Cerebrium, you need store it on the path **/persistent-storage**. So we can update the following paths above:
