...
<Note>
  Actions are recorded on every plan, including Hobby. Retention governs how far
  back the log can be read, not what is captured — upgrading a plan reveals
  history that was already recorded rather than starting an empty log.
</Note>

## Viewing the Log

Open a project in the [dashboard](https://dashboard.cerebrium.ai/login) and select **Audit Log**. Filter by date range, action, or outcome. Select a row to expand the full detail of the entry.
