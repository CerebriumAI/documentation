...
$$ VRAM\_{model} = numParams \times \#bytesPerDataType$$
...
Note that the number of instances you can have running simultaneously for each of your model is determined by the subscription plan that you are on. 
...
$$ min\_replicas = ceil(\frac{requests\_per\_min \times request\_duration}{60})$$
...
$$ min\_replicas = ceil(\frac{10 000 \times 0.5}{60})$$
$$.: min\_replicas = 84$$
