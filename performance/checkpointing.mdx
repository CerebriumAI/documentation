...
`duration_ms` runs from the snapshot being ready to this container's first call to either endpoint. It covers the restore plus however long the process took to reach that call. It excludes the time spent downloading the snapshot. Expect it to read slightly higher than the figure in the `CEREBRIUM_RESTORED` log line, which times the restore alone.
...
The runtime logs nothing when it skips a restore for this reason. The absence of the `CEREBRIUM_RESTORED` marker, or `"restored": false` from `GET /checkpoint/status`, is the only signal. An unexpected cold start after a successful checkpoint is normally this.
...
Email [support@cerebrium.ai](mailto:support@cerebrium.ai) with your app ID and the time of the failed checkpoint. Cerebrium records the underlying error on the node and matches it against that timestamp.

## Checkpoint interrupted

The checkpoint stopped before it finished, usually because the container was replaced or the node drained while the runtime was writing the snapshot.
