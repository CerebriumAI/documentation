...
<Note>
Please not this feature is currently in **Alpha** and therefore is subject to change. To start we are only making it available to certain users so please contact [support](mailto:support@cerebrium.ai) to get access.
Current limitations: 
- Only available for Nvida T4 instances
- RAM limits of 16GB
</Note>

## Intro

Many of our more hardcore users wanted finer-grain control over their code and package dependancies which is why we opened up our underlying architecture to allow engineers and data scientists to deploy
...
@app.post("/predict")
def some_function_name(item: Item):
...
You need to define a function with the decorator **@app.post("/predict")** which receives the Item object you defined above. 
...
Install the cerebrium-deploy framework script - this is not part of the offical Cerebrium package as this functionality is currently in Alpha. You need to contact our [team](mailto:support@cerebrium.ai) in order to get access to this script.
...
We don't require any linux packages so we can keep our pkglist.txt empty but lets make sure we instal the required Pyhton packages in our requirements.txt
