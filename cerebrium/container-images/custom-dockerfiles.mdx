...
2. Either a `CMD` or `ENTRYPOINT` directive must be defined in your Dockerfile OR the `entrypoint` key in your `cerebrium.toml` under `[cerebrium.runtime.docker]`. This specifies what runs when the container starts. The TOML configuration will take precedence
...
<Warning>
  ...
  under `cerebrium.dependencies.*`, as well as `shell_commands` and
