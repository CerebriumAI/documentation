...
<Tabs>
  <Tab title="Grafana Cloud">
    ...
    - **Header value:** `Basic YOUR_BASE64_STRING` (the output from the terminal command in Step 1)
  </Tab>
  <Tab title="Datadog">
    - **Header name:** `DD-API-KEY`
    - **Header value:** Your Datadog API key
  </Tab>
  <Tab title="Self-hosted Prometheus">
    - **Header name:** `Authorization` (if auth is enabled on your Prometheus, otherwise leave empty)
    - **Header value:** `Bearer your-token` (if auth is enabled)
  </Tab>
  <Tab title="Custom OTLP">
    Add the authentication headers required by your platform. You can add multiple headers using the **Add Header** button.
  </Tab>
</Tabs>
...
Click **Test Connection** in the dashboard to verify Cerebrium can reach your monitoring platform. You'll see a success or failure message with details.

If the test fails, double-check your endpoint URL and credentials from Step 1.
...
<Accordion title="API Reference">
  You can also manage metrics export programmatically. Find your Cerebrium API key in the [dashboard](https://dashboard.cerebrium.ai) under **Settings** → **API Keys**.
