...
The `replica_concurrency` parameter dictates the number of requests an app instance can handle concurrently. This is a hard limit, and an individual replica will
...
the system will scale out. It starts the number of new instances required to fulfil the in-flight requests. For example, if `replica_concurrency=1` and there are
...
During normal replica operation, this acts as a request timeout value. During replica shutdown, the Cerebrium system sends a SIGTERM signal to the replica and
waits for the specified grace period. If the instance has not stopped, the system issues a SIGKILL command and kills any active requests with a GatewayTimeout error.
...
<Note>
  ...
  implement SIGTERM handling yourself to ensure requests complete gracefully
  before termination. See the [Graceful Termination
