...
The following is the hardware available on the Cerebrium. We use a unique identfier for each GPU model:
...
| `response_grace_period` | The maximum amount of time in seconds Cerebrium should wait to issue a `SIGKILL` command to your instance after issuing a `SIGTERM` command for a scale down. This is to allow you to gracefully exit your application, or to force the instance to exit quickly. | int  | 900        |
