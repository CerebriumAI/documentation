...
In this tutorial, we will show you how to implement streaming in order to return results to your users as soon as possible with the use of SSE.
...
Below we define our predict function which will be responsible for our logic in order to stream results back from our endpoint.
...
Above we receive our inputs from the request item we defined. We then implement a TextIteratorStreamer in order to stream output from the model as its ready. Lastly and most
importantly we use the **yield** keyword in order to return output from our model as its generated.
...
<Note>Please note the end of the URL is set to **stream** instead of predict</Note>
