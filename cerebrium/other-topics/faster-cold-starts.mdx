...
<Note>
  [Checkpointing](/cerebrium/other-topics/checkpointing) (experimental)
  snapshots a warmed-up container, including GPU memory, so the next cold start
  restores that snapshot instead of loading the model again. It does not work
  for every app — vLLM's default `fork` start method is the usual
  incompatibility.
</Note>
