...
<Note>
  {" "}
  Maintain **separate development and production apps** in separate projects,
  so that you can safely test changes before going live.
</Note>
...
2. Click **Create Service Account**, name it `"GitHub Actions CI/CD"`, select an expiry date, and click **Create**.
...
> 🔒 Use _secrets_, not plain environment variables, to prevent leaking sensitive data in logs.
