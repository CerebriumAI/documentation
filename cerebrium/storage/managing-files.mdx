...
Cerebrium offers file management through a 50GB persistent volume that's available to all apps in a project. This storage mounts at `/persistent-storage` and helps store model weights and files efficiently across deployments. Note that the `/persistent-storage` directory is not directly accessible during development or outside of the Cerebrium environment - you'll need to use the Cerebrium CLI commands to manage these files.
...
The CLI provides four commands for working with persistent storage. Note that when using these commands, the Cerebrium CLI does not display the `/persistent-storage/` portion of the path in its output - it shows relative paths from the persistent storage root:
