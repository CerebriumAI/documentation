...
<Tip>
  It is better to start with a debian or ubuntu image and then add more
  complexity as development proceeds. `nvidia/cuda` images are bloated and
  affect both cold start and build times.
</Tip>
