...
I am a big fan of [Cal.com](https://cal.com) and believe the team is going to keep shipping incredible features and so wanted to build a demo using them. If you do not have an account you can create one [here](https://app.cal.com/signup).
...
We will be using OpenAI GPT3.5 for our use cases and so we need an API key from them. If you don’t have an account, you can sign up [here](https://openai.com/). You can then create an API key [here](https://platform.openai.com/settings/profile?tab=api-keys). The API key should be in the format: “sk_xxxxx”.
