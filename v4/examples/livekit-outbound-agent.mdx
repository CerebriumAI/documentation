...
Find the final code implementation and complete example in the examples GitHub repository [here](https://github.com/CerebriumAI/examples/tree/master/6-voice/7-outbound-livekit-agent).
...
Create a LiveKit account by signing up [here](https://cloud.livekit.io/login?r=%2F) (they have a generous free tier). Once the account
...
2. **Add Phone Numbers**:
   Under the “Develop” tab is the "Phone numbers" section. Navigate to “Active numbers” and purchase a number if you don’t already have one. Toll free numbers won't work for this use case, so ensure that a "Local" number exists.
...
- [Deepgram](https://www.deepgram.com/) for the Speech-to-Text service
- [OpenAI](https://openai.com/) for the LLM
- [Cartesia](https://www.cartesia.com/) for the Text-to-Speech service

Each service offers a generous free tier:

- **Deepgram**

Sign up for a Deepgram account [here](https://www.notion.so/Livekit-Outbound-agent-13ebfd664fb180c5b02bf9c71f72b23d?pvs=21). Create an API key from the dashboard.
...
Sign up for an OpenAI account [here](https://platform.openai.com/signup). Click “Dashboard” top right, then “API keys” in the left sidebar.
...
Sign up for a Cartesia account [here](https://www.notion.so/Livekit-Outbound-agent-13ebfd664fb180c5b02bf9c71f72b23d?pvs=21). After logging in, click API keys in the
left sidebar and create a new API key.

![Cartesia Dashboard](/images/examples/livekit-agent/cartesia-dashboard.png)

Finally, create a **.env** file that contains the environment variables from all the steps above.
