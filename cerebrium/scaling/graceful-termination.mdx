...
```mermaid
flowchart TD
    A[scale down event] --> B[Cortex]
    A --> C[custom runtime]
    
    B --> D[(automatically captured)]
    C --> E[(through user capture)]
    
    D --> F[SIGTERM sent]
    E --> F
    
    F --> G[wait for response_grace_period<br/>to force terminate]
    
    G --> H[exits]
    G --> I[timeout]
    
    H --> J[graceful termination]
    I --> K[SIGKILL]
    K --> L[gateway timeout errors]
```
