...
If UV encounters any issues, the system automatically falls back to pip, ensuring your deployment always succeeds.
...
- **UV_INSTALL_FAILED_FALLING_BACK_TO_PIP** - UV failed, using pip fallback
- **PIP_INSTALL_STARTED** - Standard pip installation (when `use_uv=false` or not specified)

<Warning>
While UV is compatible with most packages, some edge cases may trigger fallback to pip:

- Git dependencies with complex authentication
- Editable installs (`-e` flag)
- Legacy packages with non-standard metadata
- Private package indices (will use pip automatically)

The automatic fallback ensures these cases don't break your deployment.
