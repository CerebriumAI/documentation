...
def predict(prompt, temperature, top_p, top_k, max_tokens, frequency_penalty, run_id):
...
```curl
curl --location --request POST 'https://api.cortex.cerebrium.ai/v4/p-xxxxxx/mistral-vllm/predict' \
