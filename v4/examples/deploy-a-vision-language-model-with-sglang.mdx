...
The example builds an intelligent ad analysis system that evaluates advertisements across multiple dimensions. It scores how the advertisement relates to the business in question and how it performs on the given criteria.
...
For more details, see this [article](https://huggingface.co/blog/paresh2806/sglang-efficient-llm-workflows).
...
The VLM is [Qwen3-VL-30B-A3B-Instruct-FP8](https://huggingface.co/Qwen/Qwen3-VL-30B-A3B-Instruct-FP8), which requires significant GPU memory. The `cerebrium.toml` defines the environment, hardware, and scaling settings. This configuration uses an ADA_L40 GPU and includes:
...
For a complete reference of all available TOML settings, see the [TOML Reference](/toml-reference/toml-reference).

This example uses flashinfer as the backend, but other options like flash attention are also available.

Update `cerebrium.toml` with:
...
This example demonstrates how to leverage SGLang's structured generation capabilities to build an ad analysis system, using features like `fork()` for parallel processing and SGLang's built-in output control.

You can find the complete code for this tutorial in the [examples repository](https://github.com/CerebriumAI/examples/tree/master/5-large-language-models/7-vision-language-sglang).
