...
For teams with standardized system dependencies, text files can be used instead by adding the following to the `[dependencies.paths]` section:
...
<Warning>
  ...
  handled within the Dockerfile. The `[dependencies.*]` sections will
  be ignored.
</Warning>
