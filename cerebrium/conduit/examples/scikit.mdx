...
Now navigate to where your model code is stored. This could be in a notebook or a `.py` file.

Copy and paste our code below. This creates a simple random forest classifier on the Iris dataset.
This code could be replaced by any Sklearn model. Make sure you have
...
<Note>
  ...
  `sklearn_classifier` will return a `result` object containing the probability
...
This is due to a version mismatch between the version of scikit-learn used to train and that used in inference.
To fix this, you can either match the version of scikit-learn on Cerebrium or you can define the version of scikit-learn in a requirements file which you parse into the conduit.

To do this, create a file called `requirements.txt` and add the following line:
