...
- **config.yaml** - This is where you define all the configurations around your model such as the hardware you use, memory required, min replicas etc. Check [here]() for a full list
...
- [Custom Images](cerebrium/environments/custom-images): How to create your custom environments to run your ML Models.
- [Secrets](cerebrium/environments/using-secrets): Use secrets to authenticate with third party platforms.
- [Persistent Storage](cerebrium/data-sharing-storage/persistent-storage): Store model weights and files locally for faster access.
- [Long Running Tasks](cerebrium/deployments/long-running-tasks): Execute long running tasks in the background.
- [Streaming](cerebrium/endpoints/streaming): Stream output live back to your endpoint
