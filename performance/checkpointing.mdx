...
**Graphics and video APIs:** Vulkan, OpenGL, EGL, NVENC, and NVDEC are not supported, checkpointed or otherwise. The runtime enables only the NVIDIA `compute` and `utility` driver capabilities, so an app that requires the `graphics` or `video` capability either fails to start or fails at driver initialization. Use a CUDA-only pipeline.
