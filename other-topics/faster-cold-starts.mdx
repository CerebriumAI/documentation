...
<Note>
  Looking for memory snapshots, container snapshots, or a warm pool of preloaded containers? See [Memory Checkpointing](/other-topics/checkpointing), which snapshots a ready container's GPU and CPU memory so subsequent containers restore instantly.
</Note>
