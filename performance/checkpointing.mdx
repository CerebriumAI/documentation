...
If checkpoint creation succeeds, subsequent containers restore from that snapshot. A restored container logs a line beginning `CEREBRIUM_RESTORED`, including the snapshot size and how long restore took:

```text
CEREBRIUM_RESTORED: restored from checkpoint: 1.2 GiB in 3.2s (1288490188 bytes, 3200 ms)
```

The same figures are on `GET /checkpoint/status` and `GET /checkpoint/wait`:
...
`status` is this container's own save (`none` until it checkpoints). `restored` is whether it came up from a snapshot. They are separate because a restored container is frozen mid-`POST /checkpoint` and resumes on `GET /checkpoint/wait` — the POST body never arrives. `duration_ms` is gVisor restore time, not the time spent downloading the snapshot.
