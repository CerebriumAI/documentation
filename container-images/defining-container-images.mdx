...
To use a later Python version, use a [Dockerfile](/container-images/custom-dockerfiles).
...
<Tip>
  Start with a minimal Debian or Ubuntu base image, as CUDA images include many
  pre-installed components that increase container size. While the relationship
  isn't strictly linear, larger container sizes generally lead to longer
  cold-starts and build times. Begin with a lean base image and add only
  essential components as needed.
</Tip>
...
- `healthcheck_endpoint`: The endpoint used to confirm instance health. If unspecified, defaults to a TCP ping on the configured port. If the health check registers a non-200 response, Cerebrium considers the instance _unhealthy_ and restarts it if it does not recover in time.
