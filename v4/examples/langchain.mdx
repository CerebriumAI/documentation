...
Create a `main.py` file. This implementation accepts a YouTube video link and a question, then returns an answer with the corresponding time segment. Start by defining the request object:
...
Pydantic handles data validation. Both `url` and `question` parameters are required — missing either triggers an automatic error message.

## Video Transcription

OpenAI's Whisper model converts video audio to text, splitting it into phrase segments with timestamps to track answer sources.
...
[LangChain](https://python.langchain.com/en/latest/index.html) stores video segments in a vector store and uses a locally hosted LLM on Cerebrium to generate answers.
...
Text segments are chunked and stored in a FAISS vector store using a Hugging Face open-source model for embeddings. Running Whisper and the embeddings model on the same machine saves two network round trips, reducing latency by over 600ms.
