...
description: "Migrate from Hugging Face inference endpoints to Cerebrium with a step-by-step guide deploying a Llama 3.1 8B model on serverless GPUs."
---
...
Scaffold your application by running `cerebrium init [PROJECT_NAME]`. The initialization creates a `cerebrium.toml`. This file configures the deployment, hardware, scaling, and dependencies for your Cerebrium project. Update your `cerebrium.toml` file to reflect the following:
...
Replace `[CEREBRIUM_API_KEY]` with your Inference API key, found in your dashboard under API keys. This code sends a POST request to your deployed model's endpoint with a prompt, and prints the model's response.
...
- [Keeping models warm](/performance/faster-cold-starts)
