...
**Video encode and decode:** NVENC, NVDEC, and NVJPG run, but an app holding a video engine session cannot be checkpointed. See [Video encode and decode](#video-encode-and-decode).
...
The GPU video engines cannot be checkpointed. This covers NVENC (encode), NVDEC (decode), and NVJPG. The GPU checkpointer only captures CUDA compute state, so a live encoder or decoder session has no way to be saved and the snapshot is refused.
...
Release the video session before triggering the checkpoint, and reopen it afterwards. If the app must hold an encoder or decoder open for its whole lifetime, it cannot be checkpointed today.
