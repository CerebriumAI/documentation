...
command in your notebook or terminal. You will need the optional dependency
`onnxruntime` to run the model locally.

```bash
pip install --upgrade cerebrium[onnxruntime]
```

If you are on a GPU machine you can also install the GPU version of the
runtime instead.
...
![Onnx Postman Response](/images/quickstarts/onnx-3_1.png)
