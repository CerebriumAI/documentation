...
The extracted `cerebrium.exe` is not on your `PATH` by default. Until you add it, `cerebrium login` fails with `CommandNotFoundException`. Add the install folder to `PATH` for the current user:
