...
In order to run our custom FastAPI server, we need to tell Cerebrium how to run it. We do this by specifying the following in our cerebrium.toml file:
...
When running Pipecat at scale there are a few ways to architect your application on Cerebrium. If you are using Twilio, your container needs to respond in <1s other Twilio things there is an issue with the connection and drop the call.
The best here is to specify a large CPU instance (typically 10 CPUs and 8GB of memory) and run many concurrent Pipecat processes. Each Pipecat process, if simply using API's will use ~0.5 CPUs and so a 10 CPU instance can run 20 concurrent Pipecat processes.
machine will handle 20 concurrent calls on a single container. You can of course make these numbers large or smaller based on your expected traffic.

The second consideration is your scaling criteria - ie: when will I want to spawn up another container so subsequent calls don't incur a cold start. Here we recommend using Cerebrium's replica_concurrency
setting which states that when a container reaches a certain utilization of the total concurrent requests it can handle, scale up a container. Again, you can specify this however you like. 
